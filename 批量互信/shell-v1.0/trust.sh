...
setkey()
{
  ...
  for i in `seq $num`;do
    ...
    echo "------"$ip"------"
    sshpass -p "$pass" ssh -t $user@$ip -o StrictHostKeyChecking=no <$2 &> /dev/null
    if [ $? -eq 0 ] ; then
        echo -e "成功建立公钥和私钥"
    else
        echo "error: " $?
...
sed -i '/^ *$/d' $hostlist_file2  #去掉文件的空行

if [ ! -s $hostlist_file2 ]
then
  rm -rf authorized_keysrm
  setkey $hostlist_file1 $cmd_file   ####$hostlist_file2为空,互信hostlist_file1里面的主机
else
  ##########
  while read line
  do
    if cat $hostlist_file1 | grep "$line" >/dev/null
    then
...
#echo $hostlist_file2 >> $hostlist_file1
